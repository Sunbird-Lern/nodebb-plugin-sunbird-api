...
const contextBasesTags = '/api/context/tags'
...
const oidcPlugin = require.main.require(
  './node_modules/nodebb-plugin-sunbird-oidc/library.js'
)
...
var constants = {
  ...
  noGroupAddedMsg:
    '${group} was not added for the category id ${cid}, please add and try again.',
...
function getSBForumFunc (req, res) {
  ...
  if (id && type) {
    ...
    sbCategoryModel
      .find({ sbIdentifier: id, sbType: type })
      .then(data => {
...
async function getTagsRelatedTopics (req, res) {
  ...
  if (payload) {
    ...
    try {
      const data = await requestPromise(options)
      const releatedTopics = data.topics.filter(topic =>
        cid.includes(topic.cid)
      )
...
async function relatedDiscussions (req, res) {
  ...
  if (payload) {
    ...
    const cdata = await getResponseData(
      req,
      categoryUrl,
      createRelatedDiscussions,
      body,
      constants.post
    )
    ...
    if (cdata && cdata.payload) {
      console.log(
        'category created successfully and category id is',
        cdata.payload.cid
      )
      ...
      if (context && context.length > 0) {
        ...
        for (let i = 0; i < context.length; i++) {
          ...
          const isCidMapped = await sbCategoryModel.find({
          ...
          const mappedCids = await sbCategoryModel.find({
            sbIdentifier: context[i].identifier,
            sbType: context[i].type
          })
          ...
          if (i === context.length - 1) {
            ...
            console.log(result)
            res.send(
              responseData(req, res, createRelatedDiscussions, result, null)
            )
          }
        }
        // copy privileges from a category
        if (payload.privileges.copyFromCategory) {
          const result = await Categories.copyPrivilegesFrom(
            payload.privileges.copyFromCategory,
            cdata.payload.cid
          )
        }
        ...
        if (payload.subcategories && payload.subcategories.length > 0) {
          ...
          subcategories.forEach(async category => {
            ...
            if (category.privileges && category.privileges.copyFromCategory) {
              const result = await Categories.copyPrivilegesFrom(
                category.privileges.copyFromCategory,
                creatSubCategory.cid
              )
            }
          })
        }
        ...
        res.send(
          responseData(req, res, createRelatedDiscussions, null, contextError)
        )
      }
    } else {
...
async function groupsAndPrivileges (req, groupData) {
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      const addGroupAndPrivileges = await getResponseData(
        req,
        `${constants.createPrivileges}?_uid=${uid}`.replace(':cid', cid),
        createRelatedDiscussions,
        reqBody,
        constants.put
      )
      try {
        ...
        for (let j = 0; j < nodebbUids.length; j++) {
          ...
          const membershipUrl = `${constants.addUserIntoGroup}?_uid=${uid}`.replace(
            ':slug',
            groupSlug
          )
          getResponseData(
            req,
            membershipUrl.replace(':uid', nodebbUids[j]['nodebbUid']),
            createRelatedDiscussions,
            null,
            constants.put
          )
...
async function getResponseData (req, url, upstremUrl, payload, method) {
  try {
    ...
    const baseUrl = `${constants.http_protocal}://${req.get('host')}${apiSlug}${
      constants.apiPrefix
    }`
    const options = {
      uri: baseUrl + url,
...
async function copyPrivilegeData (req, res) {
  ...
  const privilegesCopied = await copyPrivilegesFromCategory(
    req,
    payload,
    copyPrivilages
  )
...
async function copyPrivilegesFromCategory (req, body, upstremUrl) {
  return new Promise(async (resolve, reject) => {
    ...
    categoryPrivilages.groups.forEach(async (group, index) => {
      ...
      const data = Object.keys(groupPrivileges).filter(
        x => groupPrivileges[x] === true
      )
      const reqObj = {
        privileges: data,
        groups: [group.name]
...
async function addUsers (req, res) {
  ...
  if (
    ...
    groupsList.length > 0 &&
    sunbirdUsers &&
    sunbirdUsers.length > 0
  ) {
    sunbirdUsers.forEach(async (user, index) => {
      let nodebbUid = await db.getObjectField(
  ...
  if (cid && groups && groups.length > 0) {
    ...
    if (isCidExist) {
      ...
      groups.forEach((group, index) => {
        const cgroup = groupsList.groups.filter(
          data => data.name.toLowerCase() === group.toLowerCase()
        )
        if (cgroup && cgroup.length > 0) {
          ...
          const noGroup = {
            name: group,
            message: constants.noGroupAddedMsg
              .replace('${group}', group)
              .replace('${cid}', cid)
          }
